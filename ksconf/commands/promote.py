...
Usage example:
    ...
    ksconf --target=local/props.conf default/props.conf
...
class PromoteCmd(KsconfCmd):
    help = dedent("""\
        Promote .conf settings from one file into another either in batch mode (all changes) or
        interactively allowing the user to pick which stanzas and keys to integrate.  Changes
        made via the UI (stored in the local folder) can be promoted (moved) to a version-controlled
        directory.""")
    description = dedent("""\
    ...
    def run(self, args):
        ...
        if bn_source.endswith(".meta") and bn_target.endswith(".meta"):
            ...
            if args.force:
                self.stderr.write("Promoting content across conf file types ({0} --> {1}) because the "
                                 "'--force' CLI option was set.\n".format(bn_source, bn_target))
            else:
                self.stderr.write("Refusing to promote content between different types of configuration "
        ...
        if not cfg_src:
            self.stderr.write("No settings in {0}.  No content to promote.\n".format(args.source.name))
