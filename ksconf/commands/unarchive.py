...
class UnarchiveCmd(KsconfCmd):
    help = "Install or overwrite an existing app in a git-friendly way.  " \
    ...
    def register_args(self, parser):
        ...
        parser.add_argument("--default-dir", default="default", metavar="DIR",
        ...
        parser.add_argument("--allow-local", default=False, action="store_true",
    ...
    def run(self, args):
        ...
        if len(app_name) > 1:
            self.stderr.write("The 'unarchive' command only supports extracting a single splunk app at "
        ...
        if local_files:
            ...
            if args.allow_local:
                ...
                self.stderr.write("Excluding these files by default.  Use '--allow-local' to override.")

        if not new_app_name and True:  # if not --no-app-name-fixes
            if app_name.endswith("-master"):
                self.stdout.write("Automatically dropping '-master' from the app name.  This is often "
        ...
        if mode == "upgrade":
            if is_git:
                ...
                if not existing_files:
                    self.stderr.write("App appears to be in a git repository but no files have been "
                ...
                if args.git_sanity_check == "off":
                    ...
                    del d
                    if is_clean:
                        self.stdout.write("Git folder is clean.   Okay to proceed with the upgrade.\n")
                    else:
                        self.stderr.write("Unable to move forward without a clean working directory.\n"
        ...
        if files_to_delete:
            self.stdout.write("Removing files that are no longer in the upgraded version of the app.\n")
        ...
        if is_git:
            ...
            if args.git_mode == "commit":
                ...
                if proc.returncode == 0:
                    self.stderr.write("You changes have been committed.  Please review before pushing "
